...
resource "aws_cloudfront_distribution" "daisuke_tanabe_cloudfront" {
  ...
  default_cache_behavior {
    ...
    cache_policy_id          = aws_cloudfront_cache_policy.daisuke_tanabe_root_cache_policy.id
  ...
  ordered_cache_behavior {
    ...
    cache_policy_id = aws_cloudfront_cache_policy.daisuke_tanabe_next_static_cache_policy.id
    compress               = true
    viewer_protocol_policy = "redirect-to-https"
  }
...
# Root キャッシュポリシー
resource "aws_cloudfront_cache_policy" "daisuke_tanabe_root_cache_policy" {
  name    = "RootCaching"
...
# Next static assets キャッシュポリシー
resource "aws_cloudfront_cache_policy" "daisuke_tanabe_next_static_cache_policy" {
  name    = "NextStaticAssetsCaching"
  comment = "Policy with caching enabled. Supports Gzip and Brotli compression"
